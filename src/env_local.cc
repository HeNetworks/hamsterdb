...
LocalEnvironment::LocalEnvironment()
  : Environment(), m_header(0), m_device(0), m_changeset(this),
    ...
    m_encryption_enabled(false), m_journal_compression(false),
    m_journal_compression_level(0), m_page_size(0)
{
}
            ...
            ham_u32_t flags, const ham_parameter_t *param)
{
  ...
  ham_u32_t record_compressor_level = HAM_DEFAULT_COMPRESSION_LEVEL;
  ham_u16_t dbi;
  std::string logdir;
  ...
  *pdb = 0;
  ...
  if (record_compressor)
    db->enable_record_compression(record_compressor, record_compressor_level);
